...
# 1. Diccionario con tus 20 variables corregidas
tickers_proyecto = {
    "Precio_Oro": "GC=F",          
...
# 2. Bucle de descarga utilizando el objeto Ticker individual
for nombre_variable, codigo_ticker in tickers_proyecto.items():
    try:
        ...
        # 1. Eliminamos zona horaria y forzamos a que la hora sea 00:00:00 (Normalización)
        ticket_descarga.index = ticket_descarga.index.tz_localize(None).normalize()
        
        # 2. Si por algún motivo hay dos filas el mismo día, agrupamos y nos quedamos con el último valor
        ticket_descarga = ticket_descarga.groupby(ticket_descarga.index).last()
        
        precio_cierre = ticket_descarga['Close']
        
        if nombre_variable == "Precio_Oro":
            volumen_oro_df = ticket_descarga['Volume'].to_frame(name="Volumen_Oro")
...
# 3. Combinación y sincronización de los datos de forma segura
if len(dataframes_individuales) == 0:
    ...
    if volumen_oro_df is not None:
        print("Añadiendo el volumen de negociación diario del Oro...")
        dataset_final = pd.concat([dataset_final, volumen_oro_df], axis=1)
    
    dataset_final.index = dataset_final.index.tz_localize(None)
    dataset_final = dataset_final.ffill().dropna()
    
    # --- 4. GUARDADO EN CARPETA 'Data' FUERA DE NOTEBOOK ---
    ruta_notebooks = os.path.dirname(os.path.abspath(__file__))
    carpeta_datos = os.path.join(ruta_notebooks, "..", "Data")
    nombre_archivo = os.path.join(carpeta_datos, "dataset_analisis_oro.csv")
    
    if not os.path.exists(carpeta_datos):
        os.makedirs(carpeta_datos)
        print(f"📁 La carpeta '{carpeta_datos}' no existía, así que la hemos creado automáticamente.")
